...
Development status: **v1.3.1 — bugfix release** (save version 9). Fixes the confirmed findings of the 2026-07-11 full-codebase review (`docs/REVIEW-2026-07-11.md`): progression-blocking quest/drop bugs (the tier-2→3 class wall, the unobtainable Heir of the Echo, the Arkan home-town lockout), a camp-ambush soft-lock, and a batch of combat/UI correctness fixes. Built on **v1.3 — the level-100 arc**, which extends the playable range from 40 to the archived level-100 cap: ~13 new hunting areas + 2 settlements across six regions, ~40 new monsters and bosses, tapered high-tier gear, extended tech chains, the Arkan/Estari/Eidas story spine, and a balance pass proven by simulation (at-level fights and bosses hold the difficulty contract; see the known-limitation note below). Ten regression suites cover the systems.
