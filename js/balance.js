...
var BALANCE = {
  ...
  // ==================== v1.9: Companion System (docs/SPEC-COMPANION-SYSTEM.md §2/§3) ====================
  // [revised] D0 (spec §5, APPROVED 2026-07-22): a companion is now a genuine second combatant,
  // deliberately overriding the archived "no summons in battle -- it's just 1v1" guardrail
  // (reference/forum/t-449.md, Nerevar, 2004-11-26) that the shipped Conjurer/monster-AI/champion-
  // affix code previously treated as absolute (docs/DESIGN.md §3/§4, docs/SPEC-TIER3-EXPANSION.md
  // §3a, docs/SPEC-V1.5-MONSTER-AI.md §11 -- all three updated to cite this override). Per-kind
  // HP/armor/power coefficients live as DATA in js/data/companions.js (the extension point for
  // future pets/mounts, D1); only the class-agnostic global levers below live here.

  // D1/§2.1: exactly one companion slot on the character (`character.companion = null |
  // {kindId, hp}`) -- casting a different Bind tech (js/data/techs.js summonKind) REPLACES the
  // active companion outright (D2: discard + new-at-full-HP, js/core/companion.js summon()). This
  // constant documents that "one active at a time" rule; it is enforced structurally (a single
  // object field, never an array/roster), not by a runtime count check against this number.
  COMPANION_MAX_ACTIVE: 1,

  // §2.3: Taunt duration generated by a companion's own BASIC action (the Earth Golem's Stone
  // Fist) -- while active, a monster's plain basic attack (not a tech with an explicit `target`)
  // redirects onto the companion instead of the player (js/core/battle.js monsterAct). [invented],
  // [sim-gated] (spec §3 S5: "taunt cannot perma-lock a boss into a stone wall").
  COMPANION_TAUNT_TURNS_BASIC: 2,

  // §2.4: Bulwark (tech_cmd_bulwark, Earth's command tech) REFRESHES the Golem's Taunt to this
  // longer duration on cast, on top of its own Armor buff -- js/core/battle.js useTech's buff
  // branch. [invented], [sim-gated].
  COMPANION_TAUNT_TURNS_BULWARK: 3,

  // §2.2: duration of the Fire Salamander's Burn DoT (Ember Lash's rider, refreshed not stacked --
  // js/core/companion.js act()); also the duration any future Burn source should use, so every
  // Burn in the game shares one lever. [invented], [sim-gated] (spec §3 S3: "no one-shot; DoT +
  // detonate must not exceed a Magus burst of equal investment").
  // [invented][sim-gated] ratchet — LOCKED by the lead's companion sim (2026-07-22, docs/
  // SPEC-COMPANION-SYSTEM.md §3 sim gate S4). A companion is bound and driven by the player's own
  // Anima, so the same Fear that cripples the player at the archived 5-level cutoff
  // (reference/manual/Fear.md; DESIGN.md §4 "5-down = certain death") overwhelms the companion
  // too: at//beyond this many levels BELOW the enemy the companion idles (js/core/companion.js
  // act()). WITHOUT this gate the Wind companion won 87% of +6 fights in sim, blowing straight
  // through the 5-down=certain-death contract; with it, every companion returns to ~0% there.
  COMPANION_FEAR_SUPPRESS_LEVELS: 5,
